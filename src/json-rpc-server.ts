...
import { RpcInternalException } from './exception/rpc-internal.exception';
import { RpcInvalidParamsException } from './exception/rpc-invalid-params.exception';
...
export class JsonRpcServer {
  ...
  private onRequest(request: TRequest, response: TResponse, next: () => void) {
    ...
    this.lifecycle(request, response, next).subscribe((result) => {
      this.sendResponse(
        response,
        Array.isArray(result) ? result[0].httpStatus : result?.httpStatus,
        request.body.id ? result : undefined,
      );
    });
  }
  ...
  private batchRequest(
    ...
    forkJoin(...requests).subscribe((results: ResponseBatch) => {
      const responses = results.filter((result) => {
        return result?.body && result?.body?.id !== undefined;
      });
      this.sendResponse(
        response,
        responses[0]?.httpStatus,
  ...
  private sendResponse(
    ...
    if (Array.isArray(result)) {
      body = result.map((result) => result.body);
    } else {
      body = result?.body;
    }
  ...
  private resolveResponseOrNullIfNotification(
    ...
    if (result instanceof RpcInvalidRequestException) {
      return this.wrapRPCError(body, result, HttpStatus.BAD_REQUEST);
    }
    if (result instanceof RpcMethodNotFoundException) {
      return this.wrapRPCError(body, result, HttpStatus.NOT_FOUND);
    }
